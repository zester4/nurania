            ...
            {activeTab === 'books' && (
                <Card>
                    {!selectedChapter ? (
                        <div className="space-y-4">
                            <div>
                                <label htmlFor="browse-book-select" className="block text-sm font-medium text-stone-600 mb-2">Select Collection:</label>
                                <select id="browse-book-select" value={bookViewSelectedBook} onChange={(e) => setBookViewSelectedBook(e.target.value)} className="w-full p-3 bg-stone-50 text-stone-800 rounded-lg border border-stone-300 focus:border-islamic-green focus:ring-islamic-green focus:outline-none transition">
                                    {HADITH_BOOKS.map(book => <option key={book.slug} value={book.slug}>{book.name}</option>)}
                                </select>
                            </div>
